...
import {
  ...
  scaleMotifVelocity,
  shiftMotifBeats,
} from "./motif-transform";
import type { Motif } from "@cc-music/music-ir";

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------

const makeMotif = (overrides?: Partial<Motif>): Motif => ({
  id: "test-motif",
  notes: [
...
const singleNoteMotif = (overrides?: Partial<Motif>): Motif =>
  makeMotif({ notes: [{ pitch: "A3", startBeat: 4, durationBeats: 0.5, velocity: 0.6 }], ...overrides });

const emptyNotesMotif = (overrides?: Partial<Motif>): Motif =>
  makeMotif({ notes: [], ...overrides });
...
describe("scaleMotifVelocity", () => {
  ...
  it("throws on negative factor", () => {
    expect(() => scaleMotifVelocity(makeMotif(), -1)).toThrow(
      "factor must be a nonnegative finite number",
    );
  });

  it("throws on NaN factor", () => {
    expect(() => scaleMotifVelocity(makeMotif(), NaN)).toThrow(
      "factor must be a nonnegative finite number",
    );
  });

  it("throws on Infinity factor", () => {
    expect(() => scaleMotifVelocity(makeMotif(), Infinity)).toThrow(
      "factor must be a nonnegative finite number",
    );
  });
...
describe("transform composition", () => {
  ...
  it("chained transforms all set source to transform", () => {
    const result = transposeMotif(
      stretchMotifRhythm(makeMotif(), 2),
      4,
    );
    expect(result.source).toEqual({ type: "transform" });
  });
